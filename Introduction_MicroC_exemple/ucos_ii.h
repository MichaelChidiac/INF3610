...
typedef struct os_mb {                   /* QUEUE CONTROL BLOCK                                         */
    struct os_mb   *OSMBOXPtr;              /* Link to next queue control block in list of free blocks     */
    void         *OSMBOXIn;               /* Pointer to where next message will be inserted  in   the Q  */
    INT16U         OSMBOXEntries;          /* Current number of entries in the queue                      */
    INT32U      OSMBOXDSize;
} OS_MBOX;
...
#if OS_MBOX_EN > 0
OS_EXT  OS_MBOX             *OSMBOXFreeList;              /* Pointer to list of free QUEUE control blocks    */
OS_EXT  OS_MBOX              OSMBOXTbl[OS_MAX_MBOX];        /* Table of QUEUE control blocks                   */
#endif
...
void *OSMemCopy(void *dest, const void *src, INT32U size);
